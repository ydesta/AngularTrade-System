...
//import { MemberClientRegiistrationModel } from "src/app/admin/settings/setting/shared/models/member-client-regiistration-model";
...
@Component({
  selector: "app-client-information-transfer-history",
  templateUrl: "./client-information-transfer-history.component.html",
  styleUrls: ["./client-information-transfer-history.component.scss"],
})
export class ClientInformationTransferHistoryComponent implements OnInit {
  @ViewChild(MatPaginator, { static: false }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: false }) sort: MatSort;
  formClient: FormGroup;
  formExistingClient: FormGroup;
  ...
  private createExistingClient() {
    this.formExistingClient = this.formBuilder.group({
  ...
  saveOrUpdateMemberClientInformation() {
    ...
    if (this.data.MemberClientTradeId === 0) {
      this.registerTradeService
        .saveExistingMemberClient(postData)
        .subscribe((res) => {
          if (res > 0) {
  ...
  getClintInformationByECXCode() {
    const formModel = this.formExistingClient.getRawValue();
    this.registerTradeService
      .getClintInformationByECXCode(formModel.EcxClientCode)
      .subscribe((res) => {
        if (res) {
